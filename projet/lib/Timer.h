...
Description: ce fichier est la déclaration des fonctions permettant d'utiliser les timers
...
protected:
    ...
    //Pour être compatible avec les autres timer 8bits
public:
    Timer1(/* args */);
    ~Timer1();
    void setPrescaler(uint16_t value);
    ...
    void setCompareValue1(uint16_t A, uint16_t B);
    void setTimerStartValue(uint8_t percent);
    //void setCompareValuePercent(uint8_t percentA, uint8_t percentB);
    void setInterupts(bool compareA, bool compareB, bool compareOF);
};
